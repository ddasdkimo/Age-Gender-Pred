...
class AgePredModel:
    ...
    def __init__(self,
        ...
        self.checkpoint_best = config.model + \
            "{}_best.nn".format(model_name.lower())
        self.checkpoint_last = config.model + \
            "{}_last.nn".format(model_name.lower())
    ...
    def soft_load_statedic(self, state_dict):
        ...
        for name, param in state_dict.items():
            if name in own_state:
                ...
                print(
                    '[soft_load_statedic] Unexpected key "{}" in saved state_dict'.format(name))
        missing = set.union(set(own_state.keys()) -
                            set(state_dict.keys()), set(error_layers))
        if len(missing) > 0:
            print(
                '[soft_load_statedic] keys in state_dict: "{}" not loaded!'.format(missing))
        return
            ...
            for phase in ['train', 'val']:
                ...
                for data in self.dataloaders[phase]:
                    ...
                    if self.use_gpu:
                        ...
                        gender_true = Variable(
                            gender_true.cuda(), volatile=volatile)
                        # age_rgs_true  = Variable(age_rgs_true.cuda(), volatile=volatile)
                        age_cls_true = Variable(
                            age_cls_true.cuda(), volatile=volatile)
                    else:
                        ...
                        age_cls_true = Variable(
                            age_cls_true, volatile=volatile)
                    ...
                    gender_loss_perc = 100 * \
                        (gender_loss / loss).cpu().data.numpy()
                    age_cls_loss_perc = 100 * \
                        (age_cls_loss / loss).cpu().data.numpy()
                    ...
                    weigh = np.linspace(
                        1, self.age_cls_unit, self.age_cls_unit)
                    ...
                    batch_age_tp = np.sum(
                        np.abs(age_cls_true - max_cls_pred_age) <= 2)  # if true, MAE < 5
                ...
                if phase == "val" \
                    ...
                    self.lr_rate = max(
                        self.min_lr_rate, self.lr_rate / self.lr_reduce_by)
    ...
    def getAgeGender(self,
        ...
        if not self.weight_loaded:
            ...
            # # checkpoint = torch.load(path, map_location=None if self.use_gpu else 'cpu')
            # self.soft_load_statedic(checkpoint['state_dic'])
            # # self.model.load_state_dict(checkpoint['state_dic'])

            checkpoint = torch.load(
                path, map_location=None if self.use_gpu else 'cpu')
            self.soft_load_statedic(checkpoint['state_dic'])
            print("[train_model] Params Loading Finished!")
            self.weight_loaded = True
